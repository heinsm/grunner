...
# resources setup
DATA_PAK = os.path.join(os.path.abspath(pyglet.resource.get_script_home()), 'data.pak')
if os.path.isfile(DATA_PAK):
    import atexit
    import shutil
    import tempfile
    import zipfile
    DATA_PATH = os.path.join(os.path.abspath(tempfile.gettempdir()), 'grid-runners-data')
    def cleanup():
    ...
    cleanup()
    try:
        with zipfile.ZipFile(DATA_PAK, "r") as pak:
            pak.extractall(DATA_PATH)
    except BaseException as ex:
        sys.exit("Failed to unpack data: %s" % ex)
    atexit.register(cleanup)
else:
    DATA_PATH = os.path.join(os.path.abspath(pyglet.resource.get_script_home()), 'data')
    if not os.path.isdir(DATA_PATH):
        sys.exit("Failed to load data: data.pak or data/ not found")
...
class Main(pyglet.window.Window):
    def __init__(self):
        ...
        except pyglet.gl.ContextException as ex:
            logging.error("Failed to create an OpenGL context: %s" % ex)
            sys.exit(1)
    ...
    def load_config(self):
        ...
        try:
            with open(pyglet.resource.get_settings_path("grunners.dat"), "r") as fd:
                state = json.load(fd)
                # test the data!
    ...
    def save_config(self):
        ...
        state["done"] = list(set(state["done"]))
        try:
            with open(pyglet.resource.get_settings_path("grunners.dat"), "w") as fd:
                json.dump(state, fd)
        except BaseException:
            logging.debug("failed!")
